...
router.get('/quiz/:id', adminProtect, async (req, res) => {
    try {
        ...
        res.render('admin/quiz-editor', { quiz, courses, user: req.session.user, active: 'quizzes' });
...
router.get('/quiz-details/:id', adminProtect, async (req, res) => {
    try {
        ...
        res.render('admin/quiz-details', { quiz, engagement, user: req.session.user, active: 'quizzes' });
    } catch (err) { res.status(500).send('Error: ' + err.message); }
});
...
router.get('/my-courses', adminProtect, async (req, res) => {
    try {
        await connectDB();
        const user = req.session.user;
...
router.get('/api/search-teacher', adminProtect, async (req, res) => {
...
router.get('/api/teachers-by-class', adminProtect, async (req, res) => {
    try {
        await connectDB();
...
// Create New Course
router.post('/add-course', adminProtect, (req, res, next) => {
    ...
    try {
        ...
        const newCourse = new Course({
            ...
            addedBy: req.session.user._id,
            chapters: Array.isArray(chapters) ? chapters.filter(c => c.title) : (typeof chapters === 'object' ? Object.values(chapters).filter(c => c.title) : []),
            students: [],
            rating: 5,
            totalReviews: 0
        });
        ...
        res.redirect('/admin/my-courses?success=Course created successfully');
    } catch (err) {
...
// Edit Course Page
router.get('/edit-course/:id', adminProtect, async (req, res) => {
    try {
        await connectDB();
...
// Update Course
router.post('/edit-course/:id', adminProtect, (req, res, next) => {
    const upload = router.upload;
    if (upload) upload.fields([{ name: 'thumbnail', maxCount: 1 }, { name: 'routineImage', maxCount: 1 }])(req, res, next);
    else next();
}, async (req, res) => {
    try {
        ...
        // Update chapters (preserve existing sub-assets if title hasn't changed, or just update titles)
        if (chapters) {
            ...
            course.chapters = chapterList.filter(c => c.title).map((c, idx) => {
                ...
                return {
                    title: c.title,
                    permittedTeachers: c.permittedTeachers ? (Array.isArray(c.permittedTeachers) ? c.permittedTeachers : [c.permittedTeachers]) : [],
        ...
        res.redirect('/admin/my-courses?success=Course updated successfully');
    } catch (err) {
...
// Delete Course
router.get('/course/:id/delete', adminProtect, async (req, res) => {
    try {
        await connectDB();
        const course = await Course.findById(req.params.id);
        ...
        res.redirect('/admin/my-courses?success=Course deleted successfully');
    } catch (err) {
...
router.get('/course/:id', adminProtect, async (req, res) => {
    try {
        await connectDB();
        ...
        const formattedStudents = students.map(s => {
            const enrollment = (s.enrolledCourses || []).find(e => e.course && e.course.toString() === course._id.toString());
            return {
                ...s,
        ...
        res.status(500).send('Error loading course intelligence hub:<br><pre>' + err.stack + '</pre>');
    }
});

// AJAX endpoint for live dashboard stats
router.get('/stats/live', adminProtect, async (req, res) => {
    try {
        await connectDB();
        const startOfDay = new Date();
        startOfDay.setHours(0, 0, 0, 0);
...
router.get('/academic', adminProtect, async (req, res) => {
    try {
        await connectDB();
...
// Instructors List
router.get('/instructors', adminProtect, async (req, res) => {
    try {
        await connectDB();
...
// Notification Center
router.post('/notifications/check-duplicate', adminProtect, async (req, res) => {
    try {
        await connectDB();
...
router.post('/notifications/broadcast', adminProtect, async (req, res) => {
    try {
        await connectDB();
...
router.get('/notifications/history', adminProtect, async (req, res) => {
    try {
        ...
        res.render('admin/notification-history', {
            user: req.session.user,
            logs,
            stats: logStats,
            filters: {
...
router.get('/notifications/:id', adminProtect, async (req, res) => {
    try {
        await connectDB();
        const NotificationLog = require('../models/NotificationLog');
        ...
        res.render('admin/notification-details', {
            user: req.session.user,
            log,
            active: 'notifications'
        });
...
router.patch('/notifications/:id', adminProtect, async (req, res) => {
    try {
        await connectDB();
        const NotificationLog = require('../models/NotificationLog');
...
// Delete Notification
router.delete('/notifications/:id', adminProtect, async (req, res) => {
    try {
        await connectDB();
        const NotificationLog = require('../models/NotificationLog');
...
router.post('/notifications/bulk-delete', adminProtect, async (req, res) => {
    try {
        await connectDB();
        const NotificationLog = require('../models/NotificationLog');
        const { ids } = req.body;
...
router.post('/notifications/bulk-retry', adminProtect, async (req, res) => {
    try {
        await connectDB();
        const NotificationLog = require('../models/NotificationLog');
        const pushNotificationService = require('../services/pushNotificationService');
...
router.post('/notifications/:id/retry', adminProtect, async (req, res) => {
    try {
        await connectDB();
        const NotificationLog = require('../models/NotificationLog');
        const pushNotificationService = require('../services/pushNotificationService');
...
router.get('/notifications/user-counts', adminProtect, async (req, res) => {
    try {
        await connectDB();
...
router.post('/notifications/:id/cancel', adminProtect, async (req, res) => {
    try {
        await connectDB();
        const NotificationLog = require('../models/NotificationLog');
...
router.post('/notifications/process-scheduled', adminProtect, async (req, res) => {
    try {
        await connectDB();
        const NotificationLog = require('../models/NotificationLog');
        const pushNotificationService = require('../services/pushNotificationService');
...
router.get('/question-details', adminProtect, async (req, res) => {
    try {
        ...
        res.render('admin/question-details', {
            mCQCount: mCQCount || 0,
...
router.get('/question/:id/delete', adminProtect, async (req, res) => {
    try {
        ...
        res.redirect('/admin/question-details');
    } catch (err) {
...
// GET: Edit Individual Question
router.get('/edit-question/:id', adminProtect, async (req, res) => {
    try {
        await connectDB();
        ...
        const isSuperAdmin = req.session.user.role === 'superadmin' || isMaster;
        ...
        console.error('Edit Question Page Error:', err);
        res.status(500).send('Error loading edit page');
    }
});

// POST: Update Individual Question
router.post('/edit-question/:id', adminProtect, async (req, res) => {
    try {
        await connectDB();
        const { 
            board, year, subject, classLevel, accessType, 
            questionType, topic, questionText, 
            options, optionA, optionB, optionC, optionD, 
            correctAnswerIndex, correctAnswer, explanation 
        } = req.body;
        ...
        if (questionType === 'MCQ') {
            if (options && Array.isArray(options)) {
                ...
                updateData.options = [optionA, optionB, optionC, optionD].filter(o => o !== undefined);
            }
            updateData.correctAnswerIndex = parseInt(correctAnswerIndex) || 0;
        }

        const updatedQuestion = await Question.findByIdAndUpdate(req.params.id, updateData, { new: true }).populate('addedBy', 'name profileImage');
        await logActivity(req, 'UPDATE', `Edited question: ${req.params.id}`, 'Question', req.params.id);
        
        if (req.xhr || req.headers.accept.indexOf('json') > -1) {
            return res.json(updatedQuestion);
        }
        
        res.redirect('/admin/question-details?success=Question updated');
    } catch (err) {
...
router.get('/exam-attempts', adminProtect, async (req, res) => {
    try {
        ...
        res.render('admin/exam-attempts', {
...
router.get('/note-details/:id', adminProtect, async (req, res) => {
    try {
        ...
        if (!note) return res.status(404).send('Note not found');
        res.render('admin/note-details', { note, user: req.session.user, active: 'notes' });
    } catch (err) { res.status(500).send('Error: ' + err.message); }
});
...
router.get('/note/:id/delete', contentAdminProtect, async (req, res) => {
    try {
        ...
        res.redirect('/admin/notes?success=Note deleted');
...
// GET: Question Bank Approvals Page
router.get('/question-bank-approvals', contentAdminProtect, async (req, res) => {
    try {
        await connectDB();
        ...
        console.error('Question Bank Approvals Error:', err);
        res.status(500).send('Error loading approvals page: ' + err.message);
    }
});

// POST: Approve Question Bank Entry
router.post('/question-bank/:id/approve', contentAdminProtect, async (req, res) => {
    try {
        await connectDB();
        ...
        res.redirect('/admin/question-bank-approvals?success=Question Bank approved');
    } catch (err) {
...
// POST: Reject Question Bank Entry
router.post('/question-bank/:id/reject', contentAdminProtect, async (req, res) => {
    try {
        await connectDB();
        ...
        res.redirect('/admin/question-bank-approvals?success=Question Bank rejected');
    } catch (err) {
...
// POST: Update Question Bank Access (Free vs Paid + Price) and optional status approval
...
// GET: Delete Question Bank Entry
router.get('/question-bank/:id/delete', contentAdminProtect, async (req, res) => {
    try {
        ...
        res.redirect('/admin/question-bank-approvals?success=Question Bank deleted successfully');
    } catch (err) {
...
router.get('/question-bank/:id', adminProtect, async (req, res) => {
    try {
        ...
        // Find all individual questions belonging to this bank
        const questions = await Question.find({
            bankId: bank._id
        ...
        res.render('admin/question-bank-details', {
            user: req.session.user,
            bank,
...
// GET: Archive Edit Studio
router.get('/edit-question-bank/:id', adminProtect, async (req, res) => {
    try {
        await connectDB();
        ...
        res.render('admin/edit-question-bank', {
            user: req.session.user,
            bank,
            questions,
            active: 'questions'
        ...
        res.status(500).send('Error loading edit studio');
    }
});

// POST: Update Bank Metadata
router.post('/edit-question-bank/:id', adminProtect, (req, res, next) => {
    const upload = router.upload;
    ...
    try {
        await connectDB();
        const { board, year, subject, classLevel, accessType } = req.body;
        ...
        await logActivity(req, 'UPDATE', `Updated Question Bank: ${board} ${year}`, 'QuestionBank', req.params.id);
        
        res.redirect(`/admin/edit-question-bank/${req.params.id}?success=Metadata updated`);
    } catch (err) {
...
// POST: Bulk Update Questions in Studio
router.post('/question-bank/:id/bulk-update-questions', adminProtect, async (req, res) => {
    try {
        await connectDB();
        const { questionIds, texts, correctIndices, answers, explanations } = req.body;
        
        if (!questionIds || !Array.isArray(questionIds)) {
            return res.redirect(`/admin/edit-question-bank/${req.params.id}?error=No questions to update`);
        }
        ...
        for (let i = 0; i < questionIds.length; i++) {
            ...
            // Note: Options are sent as options_0[], options_1[], etc.
            const options = req.body[`options_${i}`];
            if (options && Array.isArray(options)) {
                updateObj.options = options;
                updateObj.correctAnswerIndex = parseInt(correctIndices[i]) || 0;
            } else if (answers && answers[i] !== undefined) {
                updateObj.correctAnswer = answers[i];
            }

            await Question.findByIdAndUpdate(qId, updateObj);
        }

        await logActivity(req, 'UPDATE', `Bulk updated ${questionIds.length} questions in bank`, 'QuestionBank', req.params.id);
        res.redirect(`/admin/edit-question-bank/${req.params.id}?success=Questions saved`);
    } catch (err) {
        console.error('Bulk Update Error:', err);
        res.status(500).send('Error saving questions: ' + err.message);
    }
});
...
// GET: Question Composition Studio (Matrix / Archive Mode)
router.get('/add-question', contentAdminProtect, async (req, res) => {
    try {
        await connectDB();
        const mode = req.query.mode || 'standalone';
        res.render('admin/add-question', {
            user: req.session.user,
            mode,
            active: mode === 'archive' ? 'questions' : 'question-details'
        });
    } catch (err) {
        console.error('Error loading add-question studio:', err);
        res.status(500).send('Error loading add-question studio');
    }
});

// Bulk Question Entry (Archive Mode Support)
router.post('/add-question-full', contentAdminProtect, (req, res, next) => {
    const upload = router.upload;
    if (upload) upload.single('file')(req, res, next);
    else next();
}, async (req, res) => {
    try {
        await connectDB();
        // The form sends parallel arrays: qText[], qType[], qOptions[], qCorrectIndex[], qExplanation[], qAnswer[]
        const { 
            bankId, 
            board: rawBoard, 
            year: rawYear, 
            ...
            classLevel: rawClass,
            mode: rawMode,
            qText, qType, qOptions, qCorrectIndex, qExplanation, qAnswer, qMarks
        } = req.body;

        const qMarksArr = [].concat(qMarks || []);

        // Fix: If board/year/subject/class arrive as arrays (due to duplicate form fields), pick the first one
        const getVal = (val) => Array.isArray(val) ? val.filter(v => v !== '').shift() || '' : (val || '');
        
        const finalBoard = getVal(rawBoard);
        const finalYear = getVal(rawYear);
        const finalSubject = getVal(rawSubject);
        const finalClass = getVal(rawClass);
        const finalMode = getVal(rawMode) || 'standalone';

        if (!qText || !Array.isArray(qText)) {
        ...
        const savedQuestionsCount = qText.length;
        let mcqOptionPointer = 0;

        for (let i = 0; i < qText.length; i++) {
            const type = qType[i] || 'MCQ';
            let options = [];
            let correctAnswer = '';
            let correctAnswerIndex = 0;

            if (type === 'MCQ') {
                // MCQ options are sent in a single flat array qOptions[]
                // Each MCQ has 4 options
                options = qOptions.slice(mcqOptionPointer, mcqOptionPointer + 4);
                mcqOptionPointer += 4;
                correctAnswerIndex = parseInt(qCorrectIndex[i]) || 0;
            } else {
                // Non-MCQ answers are in qAnswer[]
                correctAnswer = qAnswer[i] || '';
            }
            ...
            const questionData = {
                ...
                correctAnswerIndex,
                correctAnswer,
                explanation: qExplanation[i] || '',
                marks: parseInt(qMarksArr[i] || 1) || 1,
                board: finalBoard,
                year: finalYear,
                subject: finalSubject,
                classLevel: finalClass,
        ...
        // Log Activity
        await logActivity(req, 'BULK_CREATE', `Added ${savedQuestionsCount} questions to ${targetBankId ? 'Bank' : 'Insights Hub'}`, 'Question');

        // Redirect logic: Mode-aware redirection for better UX
        if (finalMode === 'archive') {
            res.redirect('/admin/question-bank?success=' + savedQuestionsCount + ' questions added successfully');
        } else {
            res.redirect('/admin/question-details?success=' + savedQuestionsCount + ' questions added successfully');
        }
    } catch (err) {
...
router.get('/classes', adminProtect, (req, res) => res.redirect('/admin/video-classes'));
...
router.get('/video-classes', adminProtect, async (req, res) => {
    try {
                ...
                (chapter.recordedClasses || []).forEach(video => {
                    ...
                    if (matchesSearch && matchesSubject && matchesClass && matchesAccess && matchesCourse) {
                        ...
                        allVideos.push({
                            ...
                            assetType: 'Lesson', // Hardcoded for existing course videos
                            displayContext: 'Course Context',
        ...
        // --- Fetch Standalone Video Assets ---
        let assetsQuery = {};
        if (subjectFilter !== 'all') assetsQuery.targetSubject = subjectFilter;
        if (classFilter !== 'all') assetsQuery.targetClass = classFilter;
        if (accessFilter !== 'all') assetsQuery.accessType = accessFilter.toLowerCase();
        
        const standaloneAssets = await VideoAsset.find(assetsQuery).populate('addedBy', 'name').lean();
        standaloneAssets.forEach(asset => {
            const matchesSearch = !search || asset.title.toLowerCase().includes(search.toLowerCase());
            const matchesCourse = courseFilter === 'all' || (asset.targetCourse && asset.targetCourse.toString() === courseFilter);
            ...
            if (matchesSearch && matchesCourse) {
                ...
                allVideos.push({
                    ...
                    isStandalone: true,
                    title: asset.title,
                    courseTitle: asset.assetType === 'Trailer' ? 'Course Trailer' : 'Platform Asset',
                    chapterTitle: asset.displayContext,
                    subject: asset.targetSubject || 'Global',
                    classLevel: asset.targetClass || 'All',
                    instructor: asset.instructor || asset.addedBy?.name || 'Admin',
                    duration: asset.duration || '0:00',
                    videoPath: asset.videoPath,
                    accessType: asset.accessType,
                    assetType: asset.assetType,
                    displayContext: asset.displayContext,
                    thumbnail: asset.thumbnailPath,
                    createdAt: asset.createdAt
                });

                totalDurationMinutes += mins;
                totalViews += (asset.views || 0);
            }
        });
...
router.post('/settings', superAdminProtect, async (req, res) => {
    try {
        ...
        const { isMaintenanceMode, bkash, nagad, rocket, email, phone, whatsapp, facebook, youtube } = req.body;
...
router.post('/profile/update', adminProtect, async (req, res) => {
    try {
        await connectDB();
        ...
        if (newPassword && newPassword.trim() !== '') {
            if (!oldPassword) return res.redirect('/admin/profile?error=old_password_required');
            const isMatch = await user.comparePassword(oldPassword);
            if (!isMatch) return res.redirect('/admin/profile?error=wrong_old_password');
            if (newPassword !== confirmPassword) return res.redirect('/admin/profile?error=password_mismatch');
...
router.post('/profile/upload-avatar', adminProtect, (req, res, next) => {
...
router.post('/system-reset', superAdminProtect, async (req, res) => {
    try {
        ...
        res.redirect('/admin/settings?success=reset');
    } catch (err) {
...
router.post('/create-role', adminProtect, async (req, res) => {
    try {
        ...
        // Authorization Check
        const isSuperAdmin = req.session.user.role === 'superadmin' || req.session.user.email === process.env.SUPER_ADMIN_EMAIL;
        if (!isSuperAdmin && ['admin', 'superadmin'].includes(role)) {
            return res.render('admin/create-role', { 
                user: req.session.user, 
                error: 'Authorization Failed: You are only allowed to create Teacher, Support, or standard user roles.', 
                success: null, 
                active: 'create-role',
                name, email, phone, role: 'teacher', status, subjects
            });
        }
        ...
        res.redirect('/admin/create-role?success=Staff account created successfully.');
    } catch (err) {
...
router.post('/users/:id/role', adminProtect, async (req, res) => {
    try {
        ...
        const isMaster = currentUser.email && currentUser.email === process.env.SUPER_ADMIN_EMAIL;
        const isSuperAdmin = currentUser.role === 'superadmin' || isMaster;
        
        if (!isSuperAdmin && (role === 'admin' || role === 'superadmin')) {
            return res.status(403).json({ success: false, error: 'Only Superadmins can assign Admin or Superadmin roles.' });
        }
        
        const targetUser = await User.findById(req.params.id);
        if (!targetUser) return res.status(404).json({ success: false, error: 'User not found' });
        
        // Prevent regular admins from demoting existing admins/superadmins
        if (!isSuperAdmin && (targetUser.role === 'admin' || targetUser.role === 'superadmin')) {
            return res.status(403).json({ success: false, error: 'You do not have permission to modify this user.' });
        }
...
router.get('/deploy-video', adminProtect, async (req, res) => {
    try {
        await connectDB();
        const courses = await Course.find({}).select('title chapters classLevel').lean();
        res.render('admin/deploy-video', { user: req.session.user, courses, active: 'classes' });
    } catch (err) { res.status(500).send(err.message); }
});

// POST /add-class
router.post('/add-class', adminProtect, (req, res, next) => {
    // Dynamically use upload middleware if available
    if (router.upload) {
        return router.upload.single('video')(req, res, next);
    }
    next();
}, async (req, res) => {
    try {
        await connectDB();
        let { 
            deploymentType, // 'curriculum' or 'asset'
            title, videoPath, chapterId, duration, instructor, accessType,
            assetType, displayContext, targetSubject, targetClass
        } = req.body;
        ...
        if (!instructor && req.session.user) instructor = req.session.user.name;
        
        if (deploymentType === 'curriculum') {
            const course = await Course.findOne({ "chapters._id": chapterId });
            if (!course) return res.status(404).send('Chapter not found');

            const chapter = course.chapters.id(chapterId);
            // Inherit access type from course if possible, else default to 'paid'
            const videoAccess = course.accessType === 'free' ? 'free' : 'paid';

            chapter.recordedClasses.push({ 
                title, 
                videoPath, 
            ...
            await course.save();
            await logActivity(req, 'ADD_VIDEO', `Added curriculum video "${title}" to chapter ${chapter.title}`, 'Course', course._id);
        } else {
            // Deploy as Standalone Asset
            const newAsset = new VideoAsset({
                title, 
                videoPath, 
                description: req.body.description,
                duration, 
                instructor, 
                accessType: 'free', // Assets usually public unless specific logic
                assetType, 
                displayContext, 
                targetSubject, 
                targetClass,
            ...
            await newAsset.save();
            await logActivity(req, 'ADD_ASSET', `Deployed promotional asset "${title}" for ${displayContext}`, 'VideoAsset', newAsset._id);
        }

        res.redirect('/admin/video-classes?success=Operation successful');
    } catch (err) {
        res.status(500).send('Error processing deployment: ' + err.message);
    }
});
...
router.get('/class/:id/delete', adminProtect, async (req, res) => {
    try {
        ...
        // 1. Try deleting from Courses (Curriculum)
        ...
        if (course) {
            ...
            await course.save();
            await logActivity(req, 'DELETE_VIDEO', `Deleted curriculum video ID: ${videoId}`, 'Course', course._id);
            return res.redirect('/admin/video-classes?success=Video deleted');
        }

        // 2. Try deleting from VideoAsset (Promotional/Standalone)
        const asset = await VideoAsset.findByIdAndDelete(videoId);
        if (asset) {
            await logActivity(req, 'DELETE_ASSET', `Deleted promotional asset: ${asset.title}`, 'VideoAsset', videoId);
            return res.redirect('/admin/video-classes?success=Asset deleted');
        }

        res.status(404).send('Video or Asset not found');
    } catch (err) {
        ...
        if (action === 'delete') {
            for (const videoId of itemIds) {
                ...
                if (course) {
                    ...
                    // Check Standalone
                    await VideoAsset.findByIdAndDelete(videoId);
                }
            }
        }
...
router.post('/user/:id/complain/acknowledge', adminProtect, async (req, res) => {
    try {
        await connectDB();
        const user = await User.findById(req.params.id);
        if (!user) return res.status(404).json({ success: false, error: 'User not found' });
...
// Update Teacher Info
router.post('/user/:id/teacher-info', adminProtect, async (req, res) => {
    try {
        await connectDB();
...
// Toggle User Status
router.post('/user/:id/status', adminProtect, async (req, res) => {
    try {
        await connectDB();
